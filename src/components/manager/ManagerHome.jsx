...
// import { AddCircle, Delete, Edit, Instagram, YouTube ,ArrowUpward ,FilterList } from '@mui/icons-material';
...
//         autoClose: 5000, // Time before the toast disappears (in ms)
//       });
...
//           <div className="text-center mt-[0%] ml-[0%] mb-5">
//             {/* <button
//               onClick={() => setDrawerOpen(true)}
...
import { AddCircle, ArrowUpward, Delete, Edit, FilterList, Instagram, YouTube } from '@mui/icons-material';
import React, { useEffect, useState } from 'react';
import { useNavigate } from 'react-router-dom';
import Swal from 'sweetalert2';
...
const  ManagerHome= () => {
  ...
  useEffect(() => {
    ...
    // You can add more logic here if you need to handle the case where a token is found
  }, [navigate]); // Make sure to add 'navigate' as a
  // Followers Range options
  const followersRangeOptions = [
    { label: '0 - 1000', min: 0, max: 1000 },
    { label: '1000 - 5000', min: 1000, max: 5000 },
  ...
  const handleUpload = async (event) => {
    ...
    try {
      const response = await fetch("https://server-side-influencer-1.onrender.com/manager/managercsv", {
        method: "POST",
        body: formData,
      });
      ...
      if (response.ok) {
        ...
        toast.error(`Upload Failed: ${result.message || "Something went wrong."}`);
      }
    } catch (error) {
  ...
  const handleUpArrow = (row) => {
    ...
    console.log("Location:", row.location);
    console.log("Followers:", row.followers);
    navigate('/allinfluencer1', { state: { influencer: row } });
    // Add more logs or logic as necessary for the other fields
  };
...
<h1 className="text-xl sm:text-2xl md:text-3xl lg:text-4xl font-extrabold text-center text-black mt-[50px] mb-[50px] cursor-pointer hover:text-gray-700 transition-all duration-300 ease-in-out transform hover:scale-105 ml-[20px] sm:ml-0 break-words">
  Start Searching Influencers from a Large Database
  <span className="block text-base sm:text-sm md:text-base lg:text-lg font-normal text-gray-600 mt-0 ml-[20px] sm:ml-0 break-words">
    Discover top influencers to elevate your brand's reach and impact.
  </span>
</h1>
...
{/* CSV Upload Section */}
<div className="mt-6 flex flex-col items-center justify-center" style={{ marginBottom: '30px' }}>
...
{/* <div className="sticky top-0 z-10 bg-transparent">
  ...
  <div className="flex space-x-2">
    ...
    />
      ...
      <ToastContainer
        position="bottom-center"  // Toast messages appear at the bottom-center
