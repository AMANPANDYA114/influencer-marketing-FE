...
import { useNavigate } from "react-router-dom";
import { toast } from "react-toastify"; // Import toastify for success/error notifications
import 'react-toastify/dist/ReactToastify.css'; // Import the toastify styles

const InfluencerProfileEdit = () => {
  const navigate = useNavigate();
  const influencerId = localStorage.getItem("influencerID"); // Get influencer ID from localStorage
  ...
  // Fetch influencer data when the component mounts
  useEffect(() => {
    if (influencerId) {
      fetch(`https://server-side-influencer.vercel.app/influencer/getInfluencer/${influencerId}`)
        ...
        .then((data) => {
          if (data && data.data) {
            setFormData(data.data); // Set the form data with the fetched data
  ...
  const handleSubmit = async (e) => {
    ...
    const token = localStorage.getItem("influcertoken"); // Retrieve the token

    // Modify the formData to pass 'instagramProfile' as 'instagramURL'
    const modifiedFormData = {
      ...formData,
      instagramURL: formData.instagramProfile, // Pass the value under 'instagramURL'
    };

    // Remove the 'instagramProfile' field as it's no longer needed
    delete modifiedFormData.instagramProfile;

    try {
      const response = await fetch("https://server-side-influencer.onrender.com/influencer/updateprofile", {
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`, // Add the token in the Authorization header
        },
        body: JSON.stringify(modifiedFormData), // Send the modified form data
      });
      ...
      setTimeout(() => {
        navigate("/InfluencerProfile"); // Navigate after 3 seconds
      }, 4000); 
      } else {
...
<div className="flex justify-center items-center">
  ...
  />
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        {/* Submit Button */}
        <button type="submit" className="mt-6 w-full bg-green-500 text-white py-2 rounded-md text-lg">
          Update Profile
        </button>
      </form>
    </div>
