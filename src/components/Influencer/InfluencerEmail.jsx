...
const InfluencerEmail = () => {
  ...
  const handleNextClick = async () => {
    ...
    try {
      const response = await fetch('https://server-side-influencer-1.onrender.com/influencer/generate-otp', {
        method: 'POST',
        headers: {
      ...
      if (response.status === 200) {
        ...
        navigate('/influencerotp', { state: { email } }); // Navigate to OTP page, passing email
      } else {
        toast.error(data.message || 'Failed to send OTP');
      }
    } catch (err) {
      toast.error(`Error: OTP send failed`);
      navigate('/influencerotp', { state: { email } }); // Pass email to OTP page even if error occurs
    } finally {
      setLoading(false);
    }
  };
...
export default InfluencerEmail;
